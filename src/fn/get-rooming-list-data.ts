...
export const getRoomingListData = 
  createServerFn({
    method: 'GET',
    response: 'data',
  })
    ...
    .handler(async (ctx) => {
      ...
      if (filters) {
        ...
        if (filters.closed !== undefined) queryParams.append('closed', String(filters.closed));
        if (filters.cancelled !== undefined) queryParams.append('cancelled', String(filters.cancelled));
      }
      ...
      const data: PaginatedResponse<RoomingListData> = await res.json();
